...
,m_outFocusThreshold(200.0f) // added
,m_focus(false) // added
,m_auto_focus(true) // added
,m_id(0) // added
,m_lastManipulator(MANIPULATOR_NONE) // added
{
...
void ofxManipulator::mouseMoved(ofMouseEventArgs &mouse)
{
    ...
    // http://cvl-robot.hateblo.jp/entry/2015/09/17/115748
    ...
    // OnFocus process
    if (dist < m_outFocusThreshold * m_manipulatorScale) {
        if (m_focus) {
