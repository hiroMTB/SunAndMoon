...
CalcPosition ofxSunCalc::getSunPosition( const Poco::DateTime & date, double lat, double lon ) {
    return getSunPosition( dateToJulianDate(date), -lon * deg2rad, lat * deg2rad );
}

CalcPosition ofxSunCalc::getSunPosition( double J, double lw, double phi ) {
...
CalcPosition ofxSunCalc::getMoonPosition( const Poco::DateTime & date, double lat, double lng){
